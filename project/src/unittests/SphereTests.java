...
public class SphereTests {
    ...
    public void findIntersectionsTest() {
        ...
        l = List.of(result.get(0)._point, result.get(1)._point);
        
        assertEquals("Ray crosses sphere", exp, l);

        // TC03: Ray starts inside the sphere (1 point)
        assertEquals("Ray from inside sphere", (p2),
                sphere.findIntersections(new Ray(new Point3D(0.5, 0.5, 0), new Vector(3, 1, 0))).get(0)._point);
        ...
        assertEquals("Ray from sphere inside",
        		(new Point3D(2, 0, 0)),
                sphere.findIntersections(new Ray(new Point3D(1, -1, 0), new Vector(1, 1, 0))).get(0)._point);
        ...
        assertEquals("Line through O, ray crosses sphere",List.of(new Point3D(1, -1, 0), new Point3D(1, 1, 0)), l);

        // TC14: Ray starts at sphere and goes inside (1 points)
        assertEquals("Line through O, ray from and crosses sphere",
        		(new Point3D(1, 1, 0)),
                sphere.findIntersections(new Ray(new Point3D(1, -1, 0), new Vector(0, 1, 0))).get(0)._point );

        // TC15: Ray starts inside (1 points)
        assertEquals("Line through O, ray from inside sphere",
        		(new Point3D(1, 1, 0)),
                sphere.findIntersections(new Ray(new Point3D(1, 0.5, 0), new Vector(0, 1, 0))).get(0)._point);

        // TC16: Ray starts at the center (1 points)
        assertEquals("Line through O, ray from O",
        		(new Point3D(1, 1, 0)),
                sphere.findIntersections(new Ray(new Point3D(1, 0, 0), new Vector(0, 1, 0))).get(0)._point);
